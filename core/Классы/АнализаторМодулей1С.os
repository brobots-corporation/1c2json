...
Функция МодульИзФайла(ПолноеИмяФайла) Экспорт
	...
	ТаблицаМетодов.Колонки.Добавить("Экспорт", Новый ОписаниеТипов("Булево"));
...
Процедура ЗаполнитьПараметрыИзОписания(ТаблицаПараметров, ТекстОписания)
	...
	Для Каждого Совпадение Из Совпадения Цикл
		...
		Если Совпадение.Группы.Количество() < 4
		 ИЛИ Совпадение.Группы[1].Длина = 0 Тогда
			ТекстСообщения = "ОШИБКА!!! Не найдено ни одного параметра в описании: " + ТекстОписания;
	...
	Для Каждого СтрокаИндекса Из ПараметрыДляСортировки Цикл
		...
		Если СтрокаПараметра = Неопределено Тогда
			ТекстСообщения = СтрШаблон("ОШИБКА!!! Не найден параметр ""%1"", указанный в описании", СтрокаИндекса.Имя);
		Иначе
...
Процедура РазобратьОписание(ТаблицаМетодов, Совпадение)
	...
	Если СтрокаМетода = Неопределено Тогда
		ТекстСообщения = СтрШаблон("ОШИБКА!!! Найдено не сопоставленное описание ""%1 %2""",
			ОписаниеМетода.Вид, ОписаниеМетода.Имя);
		Сообщить(ТекстСообщения);
		Возврат;
	КонецЕсли;
